...
# --- Layout del repositorio (Clean Architecture) ----------------------------
# Este script vive en: <repo>/platform/scripts/build/build_thesis.py
# Por tanto REPO_ROOT está 3 niveles arriba.
REPO_ROOT = Path(__file__).resolve().parents[3]

# Plantillas y assets estables (capa "platform")
TEMPLATES_DIR = REPO_ROOT / "platform" / "templates" / "styles"
PLANTILLA = TEMPLATES_DIR / "plantilla_estilos.docx"
CARATULA_MANUAL = TEMPLATES_DIR / "caratula.docx"
CARATULA_MANUAL_ALT = TEMPLATES_DIR / "caratua.docx"  # legacy fallback

# Manuscrito y media (capa "content")
CONTENT_DIR = REPO_ROOT / "content"
ENTRADA = CONTENT_DIR / "manuscript" / "Documento_Tesis.md"
# resource-path para que pandoc encuentre las imágenes referenciadas en MD
MEDIA_FIGURES_DIR = CONTENT_DIR / "media" / "figures"

# Salidas y artefactos intermedios (capa "build", gitignored)
...
def ejecutar_pandoc(origen_md, destino, reference_path):
    ...
    cmd = [
        ...
        f"--resource-path={ENTRADA.parent}",
...
def aplicar_formato_cuatro_lineas_figuras_tablas(doc):
    ...
    for i in range(1, len(ch)):
        ...
        # Si numerar_captions_tablas ya insertó un SEQ Table, no reconstruir
        if any(
            instr.text and "SEQ Table" in instr.text
            for instr in prev.iter(qn("w:instrText"))
        ):
            continue

        if not titulo:
            # Fallback: solo aplicar a párrafos con estilo de caption de tabla.
            # Párrafos de texto normal no deben convertirse en captions
            # (evita que bold labels como "Dimensiones e indicadores:" se
            # numeren erróneamente como tablas).
            prev_pstyle = _pstyle_val(p_caption)
            if prev_pstyle not in {"TableCaption", "TableTitle", "Tabletitre"}:
                continue
...
def _aplicar_fuente_xml_runs(p_elem, tamano_pt: float, fuente: str):
    """Aplica fuente y tamaño a TODOS los w:r del párrafo (incluyendo dentro de w:hyperlink).

    Itera a nivel XML para no perder runs anidados en hipervínculos TOC.
    """
    NS = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
    sz_val = str(int(tamano_pt * 2))

    # Párrafo pPr/rPr (herencia de fuente por defecto)
    pPr = p_elem.find(qn("w:pPr"))
    if pPr is None:
        pPr = OxmlElement("w:pPr")
        p_elem.insert(0, pPr)
    ...
    if rPr_pPr is None:
        ...
        pPr.append(rPr_pPr)
    for tag, attr, val in [
        ("w:sz",     qn("w:val"), sz_val),
        ("w:szCs",   qn("w:val"), sz_val),
        ("w:rFonts", qn("w:ascii"), fuente),
        ("w:rFonts", qn("w:hAnsi"), fuente),
        ("w:rFonts", qn("w:cs"),    fuente),
        ("w:rFonts", qn("w:eastAsia"), fuente),
    ]:
        el = rPr_pPr.find(qn(tag))
        if el is None:
            el = OxmlElement(tag)
    ...
    # Todos los w:r en el párrafo (nivel directo Y dentro de w:hyperlink)
...
def aplicar_fuente_calibri_indices_tablas_figuras(destino):
    """Aplica Calibri 10pt a las entradas de los índices de tablas y figuras.

    Debe ejecutarse DESPUÉS de actualizar_campos_con_word, ya que las entradas
    del índice son generadas por Word al expandir los campos TOC.
    Usa iteración XML para capturar runs dentro de w:hyperlink (hipervínculos TOC).
    """
    doc = Document(str(destino))

    # Estilos que Word puede usar para entradas de listas de tablas/figuras
    estilos_toc = {"Table of Figures", "Table of Authorities"}
    ...
    for p in doc.paragraphs:
        ...
        # Detectar inicio de secciones de índice de tablas/figuras
        if texto in ("Índice de tablas", "Índice de figuras"):
            en_indice_tf = True
            continue

        # Detectar fin: llegar a otro Heading 1 o 2
        if en_indice_tf and estilo in ("Heading 1", "Heading 2"):
            en_indice_tf = False

        if (en_indice_tf or estilo in estilos_toc) and texto:
            _aplicar_fuente_xml_runs(p._p, 10, "Calibri")  # pyright: ignore[reportPrivateUsage]
            modificados += 1

    if modificados:
        doc.save(str(destino))
        print(f"  Calibri 10pt aplicado a {modificados} entrada(s) de índices de tablas/figuras.")
    else:
        print("  No se encontraron entradas de índices para ajustar fuente.")
...
def postprocesar_docx(destino):
    ...
    aplicar_fuente_calibri_indices_tablas_figuras(destino)
